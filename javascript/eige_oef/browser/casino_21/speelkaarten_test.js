...
function Speelkaartenboek() {
    ...
    this.index = 0; // om elke kaart een index geven en als volgende moet komen index verhogen met 1
...
Speelkaartenboek.prototype.schud = function (old_index, new_index) {
    var new_index = Math.floor((Math.random() * 10) + 1);
    if (new_index >= this.speelkaarten.length) {
        var k = new_index - this.speelkaartenlength;
        while ((k--) + 1) {
            this.speelkaarten.push(undefined);
        }
    }
    this.speelkaarten.splice(new_index, 0, this.speelkaarten.splice(old_index, 1)[0]);
